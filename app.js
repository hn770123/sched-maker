...
class ScheduleApp {
  ...
  createTodoBox(todo) {
    ...
    duration.textContent = `${todo.duration}h`;
  ...
  attachEventListeners() {
    ...
    if (addBtn) {
      addBtn.addEventListener('click', () => {
        ...
        const duration = parseFloat(document.getElementById('todoDuration').value);
        this.addTodo(title, duration);
      });
    }
    
    // Enterキーで追加
    const titleInput = document.getElementById('todoTitle');
    const durationInput = document.getElementById('todoDuration');
